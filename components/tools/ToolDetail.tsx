...
export function ToolDetail({ tool }: { tool: NonNullable<ToolDetailResult> }) {
  ...
  const accentColor = primaryCategory?.accent
    ? (CATEGORY_ACCENTS[primaryCategory.accent] ?? "var(--color-foreground-faint)")
  ...
  return (
    <article className="flex flex-col gap-12 lg:flex-row lg:items-start">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-8">
        ...
        <header className="flex flex-col gap-6 sm:flex-row sm:items-center">
          ...
          >
            {tool.logoUrl ? (
              ...
              <span className="text-3xl font-bold tracking-tight" style={{ color: "var(--color-foreground-muted)" }}>
                {initials}
              </span>
            )}
          </div>
        ...
        {/* Action Buttons */}
        <div className="flex flex-wrap items-center gap-4 border-b pb-8" style={{ borderColor: "var(--color-border)" }}>
          <a
        ...
        <section className="flex flex-col gap-4">
          <h2 className="text-xl font-semibold text-[var(--color-foreground)]">About {tool.name}</h2>
        ...
        {tool.screenshots.length > 0 && (
          <section className="flex flex-col gap-4">
            <h2 className="text-xl font-semibold text-[var(--color-foreground)]">Gallery</h2>
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
              {tool.screenshots.map((img) => (
                <div key={img.id} className="snap-start shrink-0 rounded-xl overflow-hidden border" style={{ borderColor: "var(--color-border)", minWidth: "300px", maxWidth: "600px" }}>
                  <Image
                    src={img.imageUrl}
                    alt={img.altText}
                    width={600}
        ...
        <div className="grid sm:grid-cols-2 gap-8">
          {tool.capabilities.length > 0 && (
            <section className="flex flex-col gap-4">
              <h2 className="text-xl font-semibold text-[var(--color-foreground)]">Capabilities</h2>
              <ul className="flex flex-col gap-3">
                {tool.capabilities.map((cap) => (
                  <li key={cap.id} className="flex gap-3 text-sm text-[var(--color-foreground-soft)]">
                    <CheckCircle2 className="h-5 w-5 shrink-0 text-[var(--color-accent)]" />
                    <div className="flex flex-col">
                      <span className="font-medium">{cap.title}</span>
                      {cap.summary && <span className="text-[var(--color-foreground-faint)] mt-0.5">{cap.summary}</span>}
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {tool.useCases.length > 0 && (
            <section className="flex flex-col gap-4">
              <h2 className="text-xl font-semibold text-[var(--color-foreground)]">Use Cases</h2>
              <ul className="flex flex-col gap-3">
                {tool.useCases.map((uc) => (
                  <li key={uc.id} className="flex gap-3 text-sm text-[var(--color-foreground-soft)]">
                    <MonitorSmartphone className="h-5 w-5 shrink-0 text-[var(--color-accent)]" />
                    <div className="flex flex-col">
                      <span className="font-medium">{uc.title}</span>
                      {uc.summary && <span className="text-[var(--color-foreground-faint)] mt-0.5">{uc.summary}</span>}
                    </div>
                  </li>
                ))}
              </ul>
      ...
      <aside className="w-full lg:w-80 shrink-0 flex flex-col gap-6">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {tool.pricingNote && (
              <span className="text-sm text-[var(--color-foreground-muted)]">{tool.pricingNote}</span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <span className="text-sm text-[var(--color-foreground-soft)]">
              {tool.companyUrl ? (
                <a href={tool.companyUrl} target="_blank" rel="noopener noreferrer" className="hover:underline hover:text-[var(--color-accent-hover)]">
                  {tool.companyName}
                </a>
              ) : (
                tool.companyName
            ...
            {tool.companyLocation && (
              <span className="text-sm text-[var(--color-foreground-muted)]">{tool.companyLocation}</span>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3">
            {tool.launchDate && (
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--color-foreground-muted)]">Launched</span>
                <span className="text-[var(--color-foreground-soft)] font-medium tabular-nums">{formatDate(tool.launchDate)}</span>
              </div>
            )}
            {tool.publishedAt && (
              <div className="flex justify-between items-center text-sm">
                <span className="text-[var(--color-foreground-muted)]">Added</span>
                <span className="text-[var(--color-foreground-soft)] font-medium tabular-nums">{formatDate(tool.publishedAt)}</span>
              </div>
            )}
          </div>
